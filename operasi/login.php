...
if(isset($_POST["btn_username"])) {
    $qry = mysqli_query($con, "SELECT username FROM operasi WHERE username='".mres($con,$_POST["username"])."'") or die (mysqli_error($con));
    ...
    $qry = mysqli_query($con, "SELECT password FROM operasi WHERE username='".mres($con,$_POST["username"])."' and password='".md5(mres($con,$_POST["password"]))."'") or die (mysqli_error($con));
    if(mysqli_num_rows($qry)>0){
        $_SESSION["operasi"]=$_POST["username"];
