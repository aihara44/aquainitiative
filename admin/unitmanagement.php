                    ...
                    <tbody>
                       <?php
                        $qry = "";
                            ...
                            $qry = mysqli_query($con, "SELECT * FROM unit INNER JOIN tingkat ON unit.tingkat = tingkat.tingkat_id ORDER BY tingkat.blok asc");
                        
                        while($row=mysqli_fetch_array($qry,MYSQLI_ASSOC)){
                            echo '<tr>';
                            echo '<td>'.$row["unit"]."</td><td>".$row["tingkat"]."</td><td><a href='addunit.php?edit_id=".$row["unit_id"]."' class='btn btn-primary'>Edit</a> | <a href='?delete_id=".$row["unit_id"]."' onclick=\"return confirm('Are you sure you want to delete this item?');\" class='btn btn-danger'>Delete</a></td>";
                            echo '</tr>';
                        }
                        ?>
