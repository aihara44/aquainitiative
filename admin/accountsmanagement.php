<?php
    ...
    $qry = mysqli_query($con, "DELETE FROM akaun WHERE akaun_id ='" .mres($con, $_GET["delete_id"])."'");
                    ...
                    <tbody>
                       <?php
                        ...
                        if(isset($_POST["btn_search"])) {
                            $qry = mysqli_query($con, "SELECT * FROM akaun WHERE username LIKE '%".mres($con,$_POST["search_text"])."%' ORDER BY akaun_id asc");
                        }else{
                            $qry = mysqli_query($con, "SELECT * FROM akaun ORDER BY akaun_id asc");
                        }
                        while($row=mysqli_fetch_array($qry,MYSQLI_ASSOC)){
                            echo '<tr>';
                            echo '<td>'.$row["akaun_id"]."</td><td>".$row["username"]."</td><td><a href='addaccount.php?edit_id=".$row["akaun_id"]."'>Edit</a> | <a href='?delete_id=".$row["akaun_id"]."' onclick=\"return confirm('Are you sure you want to delete this item?');\">Delete</a></td>";
                            echo '</tr>';
                        }
                        ?>
