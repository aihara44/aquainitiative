<?php
    ...
    $qry = mysqli_query($con, "DELETE FROM pengurusan WHERE pengurusan_id ='" .mres($con, $_GET["delete_id"])."'");
                    ...
                    <tbody>
                       <?php
                        ...
                        if(isset($_POST["btn_search"])) {
                            $qry = mysqli_query($con, "SELECT * FROM pengurusan WHERE nama_staf LIKE '%".mres($con,$_POST["search_text"])."%'");
                        }else{
                            $qry = mysqli_query($con, "SELECT * FROM pengurusan ORDER BY pengurusan_id");
                        }
                        while($row=mysqli_fetch_array($qry,MYSQLI_ASSOC)){
                            echo '<tr>';
                            echo '<td>'.$row["pengurusan_id"]."</td><td>".$row["nama_staf"]."</td><td>".$row["username"]."</td><td><a href='addmanagement.php?edit_id=".$row["pengurusan_id"]."'>Edit</a> | <a href='?delete_id=".$row["pengurusan_id"]."' onclick=\"return confirm('Are you sure you want to delete this item?');\">Delete</a></td>";
                            echo '</tr>';
                        }
                        ?>
